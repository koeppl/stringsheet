...
\usepackage{fancyhdr}
\fancyhf{} 
\fancyhead[C]{{\Large{\textbf{String Sheet \twoBeamedQuavers}}}}
\fancyhead[R]{\url{https://github.com/koeppl/stringsheet}}
...
\begin{itemize}
	...
	\item integer $:\Leftrightarrow \abs{\Sigma} = n^{\Oh{1}}$ for problem size~$n$
...
\begin{itemize}
   ...
   \item word RAM model: pointer machine model $~+~ \lg n = \Oh{w}$ access in constant time~\cite{hagerup98sorting}
   \item Transdichotomous model: word RAM model with $\lg n = \Ot{w}$~\cite{fredman93fusion}
\end{itemize}
In word RAM model: word-packing: $T'[i] = T[(i-1) \gauss{\lg_\sigma w}+1 \IC{} (i+1) \gauss{\lg_\sigma w}]$
...
\begin{itemize}
	...
	\item $\ibeg{R_i} = \arrC[P[i]] + \BWT.\rank(\ibeg{R_{i+1}} + 1, P[i]) = \LF[\ibeg{R_{i+1}}]$
	   \item $\iend{R_i} = \arrC[P[i]] + \BWT.\rank(\iend{R_{i+1}}, P[i]) = \LF[\iend{R_{i+1}}]$
\end{itemize}
